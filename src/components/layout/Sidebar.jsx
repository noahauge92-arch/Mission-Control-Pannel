...
import {
  LayoutDashboard, Zap, ScrollText, ChevronRight,
  TrendingUp, Activity,
} from 'lucide-react'
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="flex items-center justify-between mt-1">
              <span className={clsx('text-[10px] font-mono font-semibold', REGIME_COLORS[regime] || 'text-mc-muted')}>
                {regime}
              </span>
              <span className="text-[10px] text-mc-muted">{openPos} pos</span>
            </div>
            <div className="text-[9px] text-mc-subtle font-mono mt-0.5">
              UP {fmtUptime(uptimeSeconds)}
            </div>
          </>
        )}
      </div>
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <button
              ...
              className={clsx(
                'w-full flex items-center gap-2.5 px-3 py-2 rounded-md text-[13px] font-medium transition-all mb-0.5 group',
                isActive
